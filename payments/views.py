...
def process_payment(request, order_id):
    ...
    # For GET request, create or retrieve the PaymentIntent and render the payment page
    if request.method == 'GET':
        ...
        try:
            ...
            if order.stripe_id:
                ...
                # If it's already succeeded or canceled, redirect or show message
            ...
            # --- Debugging: Print client_secret to console ---
            print(f"DEBUG: PaymentIntent client_secret for Order {order.id}: {client_secret}")
            # --- End Debugging ---

            if not client_secret:
                messages.error(request, "Stripe client secret could not be generated. Please try again.")
                return redirect('cart:cart_detail')

            return render(request, 'payments/process_payment.html', {
                'order': order,
                'stripe_publishable_key': settings.STRIPE_PUBLISHABLE_KEY,
                'client_secret': client_secret, # Pass client_secret to template
            })
        except stripe.error.StripeError as e:
            # Catch specific Stripe API errors
            messages.error(request, f"Stripe API Error: {e.user_message or e.code}")
            traceback.print_exc()
            return redirect('cart:cart_detail')
        except Exception as e:
            messages.error(request, f"Error preparing payment: {str(e)}")
            # Log the full traceback in development for better debugging
            traceback.print_exc() 
    ...
    # The actual payment confirmation is handled by Stripe.js on the frontend,
    # which then redirects to the return_url. This POST block can be simplified
    # or removed if only client-side confirmation is used.
    # For now, we'll return a success, as the client-side will handle the redirect.
